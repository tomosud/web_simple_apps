...
- **慣性システム**: 
  ...
  - 質量: 3.0（適度な重さ）
  - 感度: 0.005（適度な反応）
- **スマホ対応**: passive: false、touch-action: none設定
- **直感的操作**: 地球がボールのように転がって見える

**解決した主要問題**:
1. **極地での制限**: 制限なしカメラ移動で完全解決
2. **複雑な処理**: シンプルな球面座標系で安定化
3. **操作感**: 慣性システムで滑らかな操作感を実現
4. **トラックボール感覚**: 地球がボールのように転がって見える

**参考コード**: `sample/js/main.js` のカメラ制御部分、`CAMERA_CONTROL_IMPROVEMENTS.md`

**設計方針**:
- **地球固定**: 地球は回転せず、カメラのみが移動
- **制限なし**: 極地を含む全方向へ自由移動
- **シンプル実装**: 複雑な処理を避け、保守性を重視
- **ゲーム向け**: 後のゲーム実装で有利な設計

**技術実装**:
- シンプルな球面座標系（theta, phi）
- 制限なしのカメラ移動
- 慣性システムによる滑らかな操作感
- スマホ・PC両対応のタッチ操作
